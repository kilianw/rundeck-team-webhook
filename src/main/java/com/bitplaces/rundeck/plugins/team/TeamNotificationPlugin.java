...
//    private static final String SLACK_EXT_MESSAGE_TEMPLATE_PATH = "/var/lib/rundeck/libext/templates";
	...
	@PluginProperty(title = "Notification Message Template", description = "Absolute path to a FreeMarker template that will be used to generate the notification message. "
			+ "If unspecified a default message template will be used.", required = false, defaultValue = "/templates", scope = PropertyScope.Project)
	private String messageTemplateLocation;
	@PluginProperty(title = "WebHook URL", description = "Team Incoming WebHook URL(s) To specify multiple rooms, separate with a comma.", required = true)
	...
	public boolean postNotification(String trigger, Map executionData, Map config) {
		...
		for (Object eo : executionData.entrySet()) {
			Map.Entry<String, ?> entry = (Map.Entry<String, ?>) eo;
			System.out.println("HERE KEY " + entry.getKey());
			System.out.println("HERE VAL " + entry.getValue());
			if (entry.getValue() != null) {
				System.out.println("HERE VAL " + entry.getValue().getClass());
			}
...
//            }
//        }else{
		ClassTemplateLoader builtInTemplate = new ClassTemplateLoader(TeamNotificationPlugin.class,
				messageTemplateLocation);

		File templateFolder = new File(messageTemplateLocation);

		List<TemplateLoader> loaders = new ArrayList<TemplateLoader>();

		loaders.add(builtInTemplate);
		if (templateFolder.isDirectory()) {
			try {
				FileTemplateLoader fileTemplate = new FileTemplateLoader(new File(messageTemplateLocation), false);

				loaders.add(fileTemplate);
...
//        }
		...
		TRIGGER_NOTIFICATION_DATA.put(TRIGGER_START,
				new TeamNotificationData(ACTUAL_TEAM_TEMPLATE, TEAM_MESSAGE_COLOR_YELLOW));
		TRIGGER_NOTIFICATION_DATA.put(TRIGGER_SUCCESS,
				new TeamNotificationData(ACTUAL_TEAM_TEMPLATE, TEAM_MESSAGE_COLOR_GREEN));
		TRIGGER_NOTIFICATION_DATA.put(TRIGGER_FAILURE,
				new TeamNotificationData(ACTUAL_TEAM_TEMPLATE, TEAM_MESSAGE_COLOR_RED));
		...
		final String[] urls = this.webhook_url.trim().split("\\s*,\\s*");
		for (final String url : urls) {

			String teamResponse = invokeTeamAPIMethod(url, message);
...
//        }
		...
		System.out.println("HERE message" + sw.toString());
		return sw.toString();
...
//        return urlEncode(mm);
	...
	// private String invokeSlackAPIMethod(String teamDomain, String token, String
	// message) {
	private String invokeTeamAPIMethod(String webhook_url, String message) {
		// URL requestUrl = toURL(SLACK_API_URL_SCHEMA + teamDomain + SLACK_API_BASE +
		// SLACK_API_WEHOOK_PATH + token);
...
//            connection.setRequestProperty("Content-Type", "application/json");
			...
			throw new TeamNotificationPluginException("Error putting data to Teams URL: [" + ioEx.getMessage() + "].",
					ioEx);
		}
	}
	...
	private int getResponseCode(HttpURLConnection connection) {
		try {
			...
			throw new TeamNotificationPluginException("Failed to obtain HTTP response: [" + ioEx.getMessage() + "].",
					ioEx);
		}
	}
